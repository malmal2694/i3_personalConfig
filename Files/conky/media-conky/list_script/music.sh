#!/bin/bash
## An app to display music files not listened to in conky.


# Variables: --------------------------------------------------------------------
HOME_ADDRESS="$HOME/Downloads"
MUSIC_FORMAT=".mp3\\|.wav\\|.ogg\\|.wma"
...
ls -1t > "$TMP_LIST"-ls
grep "$MUSIC_FORMAT" "$TMP_LIST"-ls > "$TMP_LIST" 2> "$NULL_ADDRESS"
...
# See More: Vim <URL:../conky0.conf#tn=music.sh>
